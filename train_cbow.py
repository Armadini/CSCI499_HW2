...
from model_cbow import ArmansSuperDuperCBOW
...
def train_epoch(
    ...
    for (labels, inputs) in tqdm.tqdm(loader):
        # put model inputs to device
        inputs, labels = inputs.to(device).long(), labels.to(device).long()
        labels = make_multihot(args, labels)
        ...
        preds = make_multihot(args, preds.cpu().numpy())
