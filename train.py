...
from books_dataloader import get_loaders
from model import ArmansSuperDuperSkipGram


def get_accuracy_score(pred_labels, target_labels):
    unique_preds = np.unique(pred_labels, axis=1)
    unique_targets = np.unique(target_labels, axis=1)
    result = []
    for i,(x,y) in enumerate(zip(unique_preds,unique_targets)):
        result.append(np.intersect1d(x,y).shape[0] / np.unique(y).shape[0])
    return np.average(result)


def make_multihot(args, labels):
    zeros = np.zeros((labels.shape[0], args.vocab_size))
...
def setup_dataloader(args):
    ...
    return get_loaders(encoded_sentences, lens, args.context_window_size, batch_size=args.batch_size), index_to_vocab
...
def setup_model(args):
    ...
    # ===================================================== #
    model = ArmansSuperDuperSkipGram(
        embedding_dim=128, vocab_size=args.vocab_size)
    return model
...
def train_epoch(
    ...
    for (inputs, labels) in tqdm.tqdm(loader):
        ...
        multi_hot_labels = make_multihot(args, labels)

        # calculate prediction loss
        loss = criterion(pred_logits.squeeze(), multi_hot_labels)
        ...
        _, inds = torch.topk(pred_logits, args.context_window_size*2)
        score = get_accuracy_score(inds.cpu().numpy(), labels.cpu().numpy())
...
if __name__ == "__main__":
    ...
    # ===================================================== #
    parser.add_argument(
        "--vocab_size", type=int, default=2000, help="size of vocabulary"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=5,
        type=int,
        help="the model predicts this many tokens to the left, and this many to the right",
    )
    parser.add_argument(
